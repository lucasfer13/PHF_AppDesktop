Public Class Constantes
    ...
    Public Const CONNECTION_STRING_REMOTO_INS = "server=192.168.1.150;port=25230;username=pethotel;pwd='ElLuAlBe???!19876';database=pethotelDB"
    ...
    WHERE u.idUsuari = {0}"
    Public Const INSERT_GUARDERIA = "INSERT INTO guarderia (cif, nom, descripcio, correu, telefon, idPC, carrer, Porta, actiu, avgRatin)
    VALUES ('{0}', '{1}', '{2}', '{3}', '{4}', {5}, '{6}', '{7}', 1, 0)"
    Public Const MODIFY_GUARDERIA = "UPDATE guarderia SET cif = '{0}', nom = '{1}',descripcio = '{2}', correu = '{3}', telefon = '{4}', idPC = {5}, carrer = '{6}', porta = '{7}'
    WHERE idGuarderia = {8}"
    Public Const DELETE_GUARDERIA = "UPDATE guarderia SET actiu = 0 WHERE idGuarderia = {0}"
    Public Const QUERY_GUARDERIES_SERVEIS = "SELECT s.nom, s.descripcio FROM serveis s
    ...
    ' Comandes vacances
    Public Const QUERY_GUARDERIES_VACANCES = "SELECT v.dataInici, v.dataFi FROM vacances v
    JOIN guarderia g ON g.idGuarderia = v.idGuarderia
    WHERE g.idGuarderia = {0}"
    Public Const DELETE_VACANCES = "DELETE FROM vacances WHERE idVacances = {0}"
    ' Querys direccions
    Public Const QUERY_DIRECCIO = "SELECT cp.cp, c.name, p.nom FROM codipostal cp
    JOIN ciutat c ON cp.idCiutat = c.idCiutat
    JOIN pais p ON p.idPais = c.idPais
    WHERE cp.idCP = {0}"
    Public Const QUERY_PAIS = "SELECT * FROM pais"
    Public Const QUERY_CP_CIUTAT = "SELECT cp.idCP, c.name, cp.cp FROM codipostal cp
    JOIN "
End Class